...
import { AlertCircle, FileText, CheckCircle2, Clock, Plus, Camera, X, Building2, User, MessageSquare } from 'lucide-react'
        ...
        try {
            ...
            const res = await fetchWithAuth('/api/disputes', {
                ...
                body: JSON.stringify({
                    ...
                    contractorId: me.contractorProfile.id,
        ...
        <div className="max-w-4xl mx-auto p-6">
            ...
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-gray-500">Phản hồi về vật tư hoặc vấn đề với chủ nhà</p>
                </div>
                <button
                    onClick={() => setShowForm(!showForm)}
                    className="bg-primary-600 text-white px-6 py-2 rounded-lg font-semibold flex items-center gap-2 hover:bg-primary-700 transition-colors shadow-lg"
                >
            ...
            {showForm && (
                <div className="bg-white p-6 rounded-2xl border-2 border-primary-50 shadow-xl mb-12 animate-in fade-in slide-in-from-top-4 duration-300">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Đơn hàng/Công trình liên quan</label>
                                <select
                                    required
                                    value={form.orderId}
                                    onChange={e => setForm({ ...form, orderId: e.target.value })}
                                    className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 focus:ring-2 focus:ring-primary-500 outline-none transition-all"
                                >
                            ...
                            <div>
                                ...
                                <div className="flex gap-2 p-1 bg-gray-100 rounded-xl">
                                    <button
                                        ...
                                        className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${form.targetType === 'STORE' ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-500'}`}
                                    >
                                        <Building2 size={16} /> Cửa hàng
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => setForm({ ...form, targetType: 'CUSTOMER' })}
                                        className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${form.targetType === 'CUSTOMER' ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-500'}`}
                                    >
                                        <User size={16} /> Chủ nhà
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-4">
                                ...
                                <div
                                    ...
                                    className={`w-24 h-24 bg-gray-100 rounded-xl flex flex-col items-center justify-center border-2 border-dashed border-gray-300 text-gray-400 cursor-pointer hover:bg-gray-200 hover:border-primary-300 hover:text-primary-400 transition-all ${uploadingImage ? 'animate-pulse pointer-events-none' : ''}`}
                                >
                                    {uploadingImage ? (
                        ...
                        <div className="flex justify-end pt-4">
                            <button
                                ...
                                className="bg-primary-600 text-white px-10 py-4 rounded-xl font-bold hover:bg-primary-700 disabled:bg-gray-400 transition-all shadow-xl"
                            >
                        ...
                        <div key={i} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded italic">
                                            {dis.type === 'CONTRACTOR_TO_STORE' ? 'vs Cửa hàng' : 'vs Khách hàng'}
                                        </span>
                                    </div>
                            ...
                            <div className="border-t border-gray-100 pt-4">
                                <details className="group">
                                    <summary className="text-xs font-bold text-gray-500 cursor-pointer list-none flex items-center gap-1 hover:text-primary-600 transition-colors uppercase tracking-widest">
                                    ...
                                    <div className="mt-4 space-y-3">
                                        <div className="max-h-[250px] overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                                            {dis.comments?.map((comment: any, idx: number) => (
                                                <div key={idx} className={`p-3 rounded-xl border ${comment.author.role === 'ADMIN' ? 'bg-blue-50 border-blue-100' : 'bg-primary-50 border-primary-100'}`}>
                                        ...
                                        <div className="relative mt-2">
                                            <input
                                                ...
                                                className="w-full bg-gray-50 border border-gray-100 rounded-lg p-2 text-sm focus:ring-1 focus:ring-primary-500 outline-none pr-10"
                                                onKeyDown={async (e) => {
                                                    if (e.key === 'Enter') {
