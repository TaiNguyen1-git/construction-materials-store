...
export async function POST(
  ...
  try {
    ...
    const project = await prisma.marketProject.findUnique({ where: { id: projectId } })
    ...
    if (existingBid) {
        return NextResponse.json({ success: false, error: 'Bạn đã đấu thầu dự án này rồi' }, { status: 400 })
    }
    ...
    if (project.customerId && project.customerId !== 'guest') {
        ...
        if (owner?.userId) {
            await saveNotificationForUser({
                ...
                message: `Có gói thầu mới trị giá ${validation.data.amount.toLocaleString()}đ cho dự án "${project.title}"`,
                data: { projectId, bidId: bid.id }
            }, owner.userId, 'CUSTOMER')
        }
    }

    return NextResponse.json({ success: true, data: bid, message: 'Đấu thầu thành công' })
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Unknown error'
    return NextResponse.json({ success: false, error: message }, { status: 500 })
  }
