...
export default function AdminProjectsPage() {
  ...
  const filteredProjects = projects.filter(project => {
    ...
    if (filters.search) {
      ...
      const customerEmail = project.customer?.user.email.toLowerCase() || ''
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg border">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Duyệt Tin</label>
            <select
                ...
                {filteredProjects.map((project) => (
                  <tr key={project.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {project.customer ? (
                        <>
                          <div className="text-sm text-gray-900 font-medium">{project.customer.user.name}</div>
                          <div className="text-xs text-gray-500">{project.customer.user.email}</div>
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-blue-50 text-blue-600 mt-1 uppercase">Đã đăng ký</span>
                        </>
                      ) : (
                        <>
                          <div className="text-sm text-gray-900 font-medium">{project.guestName || 'Khách vãng lai'}</div>
                          <div className="text-xs text-gray-500">{project.guestPhone || project.guestEmail}</div>
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-50 text-amber-600 mt-1 uppercase">Guest Submission</span>
                        </>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col gap-1">
                        <span className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(project.status)}`}>
                          {project.status.replace('_', ' ')}
                        </span>
                        {project.moderationStatus && (
                          <span className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-tighter ${project.moderationStatus === 'APPROVED' ? 'bg-emerald-100 text-emerald-800' :
                            project.moderationStatus === 'PENDING' ? 'bg-orange-100 text-orange-800 animate-pulse' :
                              'bg-red-100 text-red-800'
                            }`}>
                            {project.moderationStatus}
                          </span>
                        )}
                        <span className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase ${project.isVerified ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-500'
                          }`}>
                        ...
                        {project.moderationStatus === 'PENDING' && (
                          <>
                            <button
                              ...
                              className="text-emerald-600 hover:text-emerald-900 bg-emerald-50 p-1.5 rounded-full transition-colors"
                              title="Duyệt dự án"
                            >
