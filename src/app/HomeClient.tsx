...
const WishlistButton = dynamic(() => import('@/components/WishlistButton'), {
  ssr: false,
  loading: () => <div className="w-8 h-8 rounded-full bg-slate-100 animate-pulse" />
})
...
export default function HomeClient({
  ...
  const getSubCategories = (cat: Category) => {
    ...
    if (cat.children && cat.children.length > 0) {
      return cat.children.map((c: { name: string }) => c.name)
    }
    ...
    if (key) return FALLBACK_SUB_CATEGORIES[key]
  ...
  const getSuggestedProducts = (cat: Category) => {
    ...
    if (key) return FALLBACK_PRODUCTS[key]
    return [
      { name: `${cat.name} Loại 1`, price: '120.000₫', unit: getUnitFromProductName(`${cat.name} Loại 1`) },
      { name: `${cat.name} Dự án`, price: '115.000₫', unit: getUnitFromProductName(`${cat.name} Dự án`) },
      { name: `${cat.name} Xuất khẩu`, price: '145.000₫', unit: getUnitFromProductName(`${cat.name} Xuất khẩu`) }
    ]
  }

  /* State for banners */
  const [dbBanners, setDbBanners] = useState<Banner[]>(initialBanners)
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.length > 0) {
          ...
          const mapped = data.map((b: { imageUrl: string, tag: string, title: string, description: string, link?: string }) => ({
                        ...
                        <div className="p-4">
                          ...
                          <div className="space-y-1">
                            {TOP_SEARCHES.map((item, idx) => (
                              <button key={idx} onClick={() => setSearchQuery(item.name)} className="w-full flex items-center justify-between px-3 py-2.5 hover:bg-blue-50 rounded-lg transition-colors group" aria-label={`Tìm kiếm ${item.name}`}>
                ...
                {activeSearchTab === 'contractors' && (
                  <div className="flex-1 relative">
                    <button
                      ...
                      aria-label="Chọn khu vực tìm kiếm"
                    >
            ...
            >
              ...
              <div className="py-2 relative">
                {(categories.length > 0 ? categories : [
                  ...
                  { id: '5', name: 'Điện - Nước' },
                  { id: '6', name: 'Nội thất - Trang trí' },
                  { id: '7', name: 'Công cụ - Máy móc' }
                ]).map((cat, i) => (

                  <div
                    key={i}
                    onMouseEnter={() => setHoveredCategory(cat as Category)}
                    className="relative"
                  >
                    <Link
